...
const RUN_ID: &str = "Leviathan full slash settle";
const BOND: u64 = 500;
...
pub async fn run() {
    ...
    // The cheater posts its bond, the price of admission.
    ...
    // A stranger cannot open a dispute: run_slash is gated on the run authority.
    ...
    for _ in 0..4 {
        ...
        for client in &clients {
            ...
            let witness_proof =
                CommitteeSelection::from_coordinator(&state.coordinator, 0)
                    .unwrap()
    ...
    // The run authority convicts the cheater through the treasurer, which CPIs
    // the coordinator's slash_client with the run PDA signature.
    process_treasurer_run_slash(
        &mut endpoint,
        &payer,
        &main_authority,
    ...
    // The cheater tries to reclaim the full bond; the slashed points are
    // forfeited into the run vault before anything is returned.
    ...
    // The cheater recovers the bond minus the slashed amount; the forfeit stays
    // in the run vault as reward liquidity.
    assert_amount(&mut endpoint, &clients_collateral[cheater], BOND - SLASHING_RATE).await;
    assert_amount(&mut endpoint, &run_collateral, SLASHING_RATE).await;
}
